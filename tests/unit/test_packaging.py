...
class PackagingTests(unittest.TestCase):
    ...
    def test_archive_pdf_verifier_fails_closed_without_optional_pdf_dependency(self) -> None:
        repository = Path(__file__).resolve().parents[2]
        with tempfile.TemporaryDirectory() as raw:
            temporary = Path(raw)
    ...
    def test_extracted_package_completes_synthetic_local_journey(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as raw:
            ...
            script = textwrap.dedent(
                ...
                write_text_pdf(workspace.root / record.resume_pdf); hashes = collect_local_artifacts(workspace, record).hashes
                ...
                receipt = QualityReceipt.all_passed(2, None).as_dict(); receipt['resume_layout'] = measure_resume_layout(workspace.root / record.resume_pdf); receipt['bindings'] = {{'profile_hash': record.profile_hash, 'criteria_hash': record.criteria_hash, 'writing_preferences_hash': record.writing_preferences_hash, 'packet_options': dict(record.packet_options), 'role_instructions': record.role_instructions, 'posting_snapshot_hash': 'b' * 64, 'draft_hashes': {{'resume': 'a' * 64}}, 'final_pdf_hashes': dict(hashes)}}
                manifest = advance_packet(manifest, 'JOB-000001', 'quality_checked', receipt, workspace=workspace); manifest = advance_packet(manifest, 'JOB-000001', 'saved', {{'artifact_hashes': hashes}}); manifest = complete_local_delivery(workspace, manifest, 'JOB-000001', occurred_at='2026-09-11T12:15:00Z')
